...
function Room() {
  ...
  const { data, error, isLoading } = useMoralisQuery(
    "Messages",
    (query) => query.equalTo("roomID", id).ascending("createdAt"),
    [],
    { live: true }
  );
        ...
        <>
          <div className="w-[30%] h-full bg-blue-200">
            <input
              ...
              className="mt-10 ml-4 rounded-md py-2 pl-2 pr-40 focus:outline-none w-[20%]"
            />
            <button
              onClick={AddtoRoom}
            ...
            <div className="flex-col w-full ml-4 mt-4">
              {roomdetails[0].attributes.people.map((item) => {
                return (
                  <>
                    <div className="flex align-middle my-2 bg-white p-2 rounded-md w-[70%]">
                      <Image
          ...
          <div className='bg-yellow-400 w-[70%] h-full'> 
